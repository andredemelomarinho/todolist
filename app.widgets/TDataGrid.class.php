    ...
    public function addItem($object)
    {
        ...
        if ($this->actions)
        {
            ...
            foreach ($this->actions as $action)
            {
				...
				$field1=$action->getField1();
				$field2=$action->getField2();
				$field3=$action->getField3();

                // obtém o campo do objeto que será passado adiante
                $key    = $object->$field;
				$ativo= $object->$field1;
				$vendedor= $object->$field2;
				$cliente= $object->$field3;
                ...
                if ($image)
                {
                    ...
                    $image=new TImage("app.images/$image",$label);
                    $link->add($image);
                }
                else
                {
