<?php
/*
 * classe VendedoresList
 * cadastro de vendedores: contém o formuláro e a listagem
 */
class UsuarioLogin extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->form = new TForm('form_vendedores');
		...
		// carrega os vendedores do banco de dados
        TTransaction::open('my_livro');

        // instancia um repositório de Vendedor
        $repository = new TRepository('Vendedor');
       ...
       // cria um botão de ação (salvar)
        ...
        $this->form->setFields(array($codigo, $nome, $save_button));
    ...
    function onReload()
    {
        ...
        $vendedores = $repository->load($criteria);
        $this->datagrid->clear();
        if ($vendedores)
        {
            ...
            foreach ($vendedores as $vendedor)
            {
                ...
                $this->datagrid->addItem($vendedor);
            }
        }
    ...
    function onSave()
    {
        // inicia transação com o banco 'pg_livro'
        TTransaction::open('pg_livro');
        // obtém os dados no formulário em um objeto Vendedor
        $vendedor = $this->form->getData('Vendedor');
        // armazena o objeto
        $vendedor->store();
        // finaliza a transação
        TTransaction::close();
