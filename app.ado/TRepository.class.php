...
final class TRepository
{
    ...
    */
	...
	if($classe=='pagamentovenda')
	{
	$coluna='distinct venda_id';
	}
    ...
    */
	...
	function sum(TCriteria $criteria,$coluna)
    {
        // instancia instrução de SELECT
        $sql = new TSqlSelect;
        $sql->addColumn('sum('.$coluna.') as '.$coluna);
        $sql->setEntity(constant($this->class.'::TABLENAME'));
        // atribui o critério passado como parâmetro
        $sql->setCriteria($criteria);
        // obtém transação ativa
	...
	function sum_group(TCriteria $criteria,$coluna,$group)
    {
        // instancia instrução de SELECT
        $sql = new TSqlSelect;
        $sql->addColumn('sum('.$coluna.') as '.$coluna.','.$group);
        $sql->setEntity(constant($this->class.'::TABLENAME'));
        // atribui o critério passado como parâmetro
        $sql->setCriteria($criteria);
        // obtém transação ativa
